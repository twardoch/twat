# twat_search Implementation Plan
...
**Goal:** Preserve multi-engine search while isolating embedded scraper code and improving docs/tests.

**Architecture:** Preserve the standard twat plugin contract: PyPI package `twat-search`, import module `twat_search`, entry point `search = twat_search`, public `main()`, explicit `__all__`, README examples, and tests. Keep provider/API integrations behind thin clients and keep user-facing domain functions small.
...
- Path: `plugins/repos/twat_search`.
- Entry point name: `search`.
- Tier: `complex`.
...
- Modify if needed: `plugins/repos/twat_search/pyproject.toml`
- Modify if needed: `plugins/repos/twat_search/src/twat_search/__init__.py`
- Modify if needed: `plugins/repos/twat_search/README.md`
- Modify/Create if needed: `plugins/repos/twat_search/tests/`
