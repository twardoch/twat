...
**Goal:** Preserve multi-provider upload API while cleaning provider typing, README, tests, and optional backend boundaries.

**Architecture:** Preserve the standard twat plugin contract: PyPI package `twat-fs`, import module `twat_fs`, entry point `fs = twat_fs`, public `main()`, explicit `__all__`, README examples, and tests. Keep provider/API integrations behind thin clients and keep user-facing domain functions small.
...
- Modify if needed: `plugins/repos/twat_fs/pyproject.toml`
- Modify if needed: `plugins/repos/twat_fs/src/twat_fs/__init__.py`
- Modify if needed: `plugins/repos/twat_fs/README.md`
- Modify/Create if needed: `plugins/repos/twat_fs/tests/`
