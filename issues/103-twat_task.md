...
**Goal:** Keep Prefect/task orchestration plugin focused and normalized.

**Architecture:** Preserve the standard twat plugin contract: PyPI package `twat-task`, import module `twat_task`, entry point `task = twat_task`, public `main()`, explicit `__all__`, README examples, and tests. Keep provider/API integrations behind thin clients and keep user-facing domain functions small.
...
- Modify if needed: `plugins/repos/twat_task/pyproject.toml`
- Modify if needed: `plugins/repos/twat_task/src/twat_task/__init__.py`
- Modify if needed: `plugins/repos/twat_task/README.md`
- Modify/Create if needed: `plugins/repos/twat_task/tests/`
